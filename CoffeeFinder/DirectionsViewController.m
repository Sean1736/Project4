...
-(void)getDirectionsFrom:(CLLocationCoordinate2D)sourceCoordinate withDestination:(CLLocationCoordinate2D)destinationCoordinate{
    ...
    MKPlacemark *detinationPlacemark = [[MKPlacemark alloc]initWithCoordinate:destinationCoordinate addressDictionary:nil];
    MKMapItem *destinationMapItem = [[MKMapItem alloc]initWithPlacemark:detinationPlacemark];
    ...
    [directions calculateDirectionsWithCompletionHandler:^(MKDirectionsResponse * _Nullable response, NSError * _Nullable error) {
        ...
        for (int i; i < route.steps.count; i++) {
